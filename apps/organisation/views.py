...
from courses.models import Course, Video, Lesson
from .forms import UserAskForm
from operation.models import UserFavorite
from utils.LoginJudge import LoginRequiredMixin
from courses.forms import ForumResourceForm
...
class TeacherCoursesView(View):
    ...
    def get(self, request):
        ...
        return render(request, 'teacher_courses.html', {'courses': courses,
                                                        'current_page': 'teacher_courses'})


class TeacherForumsView(View):
    """
    教师管理的论坛
    """
    def get(self, request):
        courses = Course.objects.filter(teacher=request.user)
        forum_list = [course.forum_set.all() for course in courses]
        forums = reduce(lambda x, y: x | y, forum_list)
        return render(request, 'teacher_forums.html', {'forums': forums,
                                                       'current_page': 'teacher_forums'})


class UploadCourse(View):
    def get(self, request):
        courses = Course.objects.filter(teacher=request.user)
        return render(request, 'teacher_upload_course.html', {'courses': courses})


class UploadCourse2(View):
    def get(self, request, course_id):
        course = Course.objects.get(id=course_id)
        lessons = course.lesson_set.all()
        return render(request, 'teacher_upload_course2.html', {'lessons': lessons,
                                                               'course': course})
    ...
    def post(self, request, course_id):
        ...
        video = Video()
        video.lesson = Lesson.objects.get(id=lesson_id)
        video.name = video_name
        video.url = video_url
        video.save()
        return render(request, 'usercenter-info.html', {'current_page': 'info'})