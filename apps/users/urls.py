...
from .views import UserInfoView, ModifyUserImageView, UpdatePwdView, SendVerifyCodeView, UpdateEmailView, \
    UserCourseView, UserFavCourseView, UserFavTeacherView, UserFavOrgView, UserSystemMessageView, QuestionnaireView, \
    UserQuestionView, UserOrderView
...
urlpatterns = [
    ...
    # 用户收到的系统信息
    url(r'^message/$', UserSystemMessageView.as_view(), name="user_system_message"),
    # 用户的问答
    url(r'^question/$', UserQuestionView.as_view(), name='user_question'),
    # 用户的订单
    url(r'^order/$', UserOrderView.as_view(), name='user_order'),

    url(r'^questionnaire/$', QuestionnaireView.as_view(), name="user_questionnaire"),
