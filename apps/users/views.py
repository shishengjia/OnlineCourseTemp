...
from operation.models import UserFavorite, UserCourse, UserMessage, Order, Questionnaire
...
from operation.forms import QuestionnaireForm
            ...
            if user is not None:  # 用户名或密码是否正确
                if user.is_active:  # 用户是否处于激活状态
                    ...
                    if user.is_first_login:
                        ...
                        user.save()
                        return render(request, 'questionnaire.html', {'types': types})
...
class UserQuestionView(View):
    ...
    def get(self, request):
        ...
        return render(request, 'usercenter_myquestion.html', {'questions': questions,
...
class UserOrderView(View):
    def get(self, request):
        ...
        return render(request, 'usercenter_myorder.html', {'orders': orders,
                                    'current_page': 'order'})
...
class QuestionnaireView(LoginRequiredMixin, View):
    ...
    def post(self, request):
        ...
        types = Type.objects.filter(name__in=type_list)

        if request.POST.get('like_answer', '') == 'on':
            like_answer = True

        questionnaire = form.save(commit=False)
        questionnaire.user = request.user
        questionnaire.is_like_answer = like_answer
        questionnaire.save()
        for type in types:
            questionnaire.type.add(type)

        # 将用户的兴趣特征下来
        redis = Redis(request.user.username)
        for type in types:
            redis.increase(type.name, 5)
        # 注意这里需要重定向，因为直接跳转无法携带首页所需要的数据
        return HttpResponseRedirect(reverse("index"))
